...
export function PaceTrackerCard({ pace, visaType }: PaceTrackerCardProps) {
  ...
  const badge = hasGoal ? formatPaceDelta(delta, pct, status) : undefined;
  ...
  return (
    <StatCardWrapper
      icon={hasGoal ? config.icon : TrendingUp}
      iconVariant={hasGoal ? config.iconVariant : 'info'}
      title="Pace Tracker"
      tooltip={
      ...
      badge={badge}
      badgeVariant={hasGoal ? config.badgeVariant : undefined}
    >
      {hasGoal ? (
        <>
          {/* Status label */}
        ...
        <>
          ...
          <div className="mt-auto">
            <span className="text-xs text-muted-foreground flex items-center gap-1">
              ...
              Average pace · No day requirement
            </span>
          </div>
        </>
      )}
