...
export function useWeekFormState(
  ...
  const datesWithHours = useMemo(() => {
    ...
    return existingHours
      ...
      .map((entry) => new Date(entry.workDate));
  }, [existingHours]);
  ...
  const submit = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const weekEntries: Array<{ workDate: string; hours: number }> = [];
      ...
      // Execute the mutation - server handles upsert/delete logic
      ...
      weekEntries.forEach((e) => {
        newInitialHours[e.workDate] = e.hours;
      });
  ...
  useEffect(() => {
    ...
    existingHours.forEach((entry) => {
      if (weekDateKeys.includes(entry.workDate)) {
