...
export function AddVisaForm({ onSubmit, onCancel, isSubmitting, onError }: AddVisaFormProps) {
  const [selectedVisaType, setSelectedVisaType] = useState<VisaType | null>(null)
  const { availableVisas, loading, error, hasAvailableVisas } = useAvailableVisas()
  ...
  const handleSubmit = async (data: CreateVisaFormData) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error : new Error('Failed to add visa')
  ...
  if (error) {
    return (
      <div className="text-center p-8">
        ...
        <p className="text-sm text-destructive">Error loading available visas: {error}</p>
    ...
    return (
      <div className="text-center p-8">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          You have already added all available Working Holiday Visas to your account.
        </p>
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
        <FormField
          ...
          render={() => (
            <FormItem>
              <FormLabel className="text-lg font-semibold">Select Visa Type</FormLabel>
              <FormDescription>
                Choose which Working Holiday Visa you want to track ({availableVisas.length} available)
              </FormDescription>
              <FormControl>
